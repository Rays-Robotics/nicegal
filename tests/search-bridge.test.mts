...
interface Fixture {
  calls: Array<{ request: Record<string, unknown>; signal: AbortSignal; resolve: (value: unknown) => void }>;
...
function fixture(): Fixture {
  const calls: Array<{ request: Record<string, unknown>; signal: AbortSignal; resolve: (value: unknown) => void }> = [];
  registerBackendIpc({
    status: { ready: true, error: null },
    isTrustedSender: () => true,
    client: { search: (request: Record<string, unknown>, signal: AbortSignal) =>
  ...
  const search = (metadata: Record<string, unknown> = {}): Promise<unknown> =>
    handlers.get("backend:search")!({ sender }, { query: "cat", type: "simple", root: process.cwd(), ...metadata });
...
test("same session lanes coexist; newer sessions and same-lane replacements abort selectively", async () => {
  ...
  assert.deepEqual(settled.map((result) => result.status), ["rejected", "rejected", "rejected", "fulfilled"]);
});
