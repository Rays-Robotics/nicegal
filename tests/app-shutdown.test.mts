...
test("quit awaits an in-flight provider fallback shutdown without respawning the backend", async (t) => {
  ...
  const sources: Record<string, string> = {
    ...
    "./updates": `export function startUpdates() { const s = globalThis.__shutdownMocks.state; return {
  ...
  assert.equal(state.quits, 0, "the on-quit updater cannot run before the backend exits");
  stopped.resolve();
